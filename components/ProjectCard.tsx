...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  useEffect(() => {
    const calculate = () => {
      ...
      setAsciiScale(
        asciiWidth > containerWidth ? containerWidth / asciiWidth : 1,
      );
    };
    calculate();
  ...
  return (
    <Link
      ...
      className="group flex cursor-pointer flex-col overflow-hidden rounded-xl border border-surface0 bg-transparent shadow-lg transition-all hover:border-accent"
    >
      <ViewTransition name={`project-terminal-${slugify(project.title)}`}>
        <div className="terminal-morph-el relative aspect-[5/4] w-full overflow-hidden bg-transparent">
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <Terminal
              sequence={false}
              className="h-full w-full max-w-none border-surface1 bg-mantle transition-transform duration-300 ease-out group-hover:scale-[1.05]"
            >
              <div
                ref={containerRef}
                className="h-20 sm:h-24 lg:h-28 overflow-hidden text-[0.55rem] sm:text-xs lg:text-sm leading-[0.7rem] sm:leading-tight lg:leading-normal"
              >
                <div
                  ref={asciiRef}
                  style={{
                    transform: `scaleX(${asciiScale})`,
              ...
              <TypingAnimation
                startOnView
                delay={300}
                className="mt-1 sm:mt-2 lg:mt-4 text-ctp-text"
              >
                $ cat TAGLINE
              </TypingAnimation>
              <AnimatedSpan
                startOnView
                delay={1300}
                className="text-subtext0 mt-1 whitespace-normal break-words text-xs sm:text-sm overflow-hidden"
              >
                {project.tagline}
